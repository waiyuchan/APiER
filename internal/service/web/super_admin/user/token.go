...
import (
	...
	"apier/internal/http/middleware/custom_jwt"
...
func CreateUserFactory() *userToken {
	return &userToken{
		userJwt: custom_jwt.CreateJWT(variable.ConfigYaml.GetString("Token.JwtTokenSignKey")),
	}
}

type userToken struct {
	userJwt *custom_jwt.JwtSign
}
...
func (u *userToken) GenerateToken(userid int64, username string, phone string, expireAt int64) (tokens string, err error) {
	...
	customClaims := custom_jwt.Payload{
		UserId:   userid,
		UserName: username,
		// 特别注意，针对前文的匿名结构体，初始化的时候必须指定键名，并且不带 custom_jwt. 否则报错：Mixture of field: value and value initializers
...
//	customClaims, code := u.isNotExpired(token, variable.ConfigYaml.GetInt64("Token.JwtTokenRefreshAllowSec"))
//	switch code {
...
//	if newToken, err = u.userJwt.RefreshToken(oldToken, variable.ConfigYaml.GetInt64("Token.JwtTokenRefreshExpireAt")); err == nil {
...
//func (u *userToken) isNotExpired(token string, expireAtSec int64) (*custom_jwt.Payload, int) {
...
//		if variable.ConfigYaml.GetInt("Token.IsCacheToRedis") == 1 {
...
//func (u *userToken) ParseToken(tokenStr string) (CustomClaims custom_jwt.Payload, err error) {
//	if customClaims, err := u.userJwt.ParseToken(tokenStr); err == nil {
//		return *customClaims, nil
//	} else {
//		return custom_jwt.Payload{}, errors.New(custom_errors.ErrorsParseTokenFail)
//	}
//}
